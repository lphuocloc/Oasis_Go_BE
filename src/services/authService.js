...
class AuthService {
  ...
  async forgotPassword({ email }) {
    ...
    try {
      await sendOTPEmail(email, otp);
      return { message: "OTP reset pass đã được gửi" };
    } catch (error) {
      throw new Error("Failed to send verification email. Please try again.");
    }
  } /**
   * Xác thực OTP Reset để lấy Reset Token
   */
  async verifyResetOtp({ email, otp }) {
    ...
    // Tạo Reset Token (chỉ dùng cho việc reset pass trong 10 phút)
    ...
    // Xóa OTP ngay lập tức để bảo mật (chống dùng lại)
  ...
  /**
   * Đặt mật khẩu mới bằng Reset Token
   */
  async resetPassword({ resetPasswordToken, newPassword }) {
    ...
    try {
      ...
      if (decoded.purpose !== "password_reset") {
        const error = new Error("Token không hợp lệ cho mục đích này");
        error.statusCode = 401;
        throw error;
      }
