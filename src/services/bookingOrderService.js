...
class BookingOrderService {
  ...
  _assertVoucherEligibility(order, voucher, now = new Date()) {
    ...
    if (voucher.usage_limit !== null && voucher.usage_limit !== undefined) {
      if (
        Number(voucher.usage_count || 0) >= Number(voucher.usage_limit || 0)
      ) {
  ...
  async _findOrderIdsByPodIds(podIds) {
    ...
    return [
      ...new Set(
        bookings.map((booking) => String(booking.order_id)).filter(Boolean),
      ),
    ];
  }
  ...
  async _releaseBookingResources(bookingIds, session = null) {
    ...
    const timeSlotIds = bookingSlots.map(
      (bookingSlot) => bookingSlot.time_slot_id,
    );
    ...
    await BookingSlot.deleteMany({ booking_id: { $in: bookingIds } }).session(
      session,
    );
  }
  ...
  async _resolveBookingAmounts(bookings = [], session = null) {
    ...
    const bookingAmounts = bookings.map((booking) => {
      ...
      const hasDetailAmount = Number.isFinite(
      ...
      return {
        ...
        source: hasDetailAmount
          ? "BOOKING_PRICING_DETAIL"
          : "BOOKING_TOTAL_PRICE_FALLBACK",
        pricing_rule_id: detail?.pricing_rule_id || null,
        applied_modifier: hasDetailAmount
          ? Number(detail?.applied_modifier || 1)
          : null,
      };
    });

    const totalAmount = bookingAmounts.reduce(
  ...
  async _calculateRefundForBookings(
    bookings = [],
    ...
    const amountResolution = await this._resolveBookingAmounts(
      eligibleBookings,
      session,
    );
    ...
    const refundAmount = Number(
      (refundableBaseAmount * REFUND_RATE_BEFORE_48H).toFixed(2),
    );
  ...
  async _recalculateOrderPricingAfterPartialCancel(order, session = null) {
    ...
    const existingBookingVoucher = await BookingVoucher.findOne({
      order_id: order.id,
    })
      .session(session)
    ...
    if (existingBookingVoucher?.voucher_id) {
      const voucher = await Voucher.findOne({
        id: existingBookingVoucher.voucher_id,
      })
        .session(session)
        .select(
          "id discount_type discount_value max_discount min_booking_value",
        );

      const minBookingValue = Number(voucher?.min_booking_value || 0);
      const stillEligibleByMinValue =
        voucher && recalculatedBase >= minBookingValue;
      ...
      if (stillEligibleByMinValue) {
        ...
        await BookingVoucher.deleteOne({
          id: existingBookingVoucher.id,
        }).session(session);
  ...
  async _creditWalletWithRefund({
    ...
    const walletBalanceAfter = Number(
      (walletBalanceBefore + totalAmount).toFixed(2),
    );
    ...
    if (walletTransactionsToCreate.length > 0) {
      createdWalletTransactions = await WalletTransaction.create(
        walletTransactionsToCreate,
        {
          session,
          ordered: true,
        },
      ...
      const txResult = await session.withTransaction(async () => {
        ...
        if (!user_id || !cluster_id || !start_time || !end_time) {
          const error = new Error(
            "Missing required fields: user_id, cluster_id, start_time, end_time",
          );
        ...
        const slotDurationMinutes =
          cluster.slot_duration_minutes || DEFAULT_SLOT_DURATION_MINUTES;

        // Validate minimum booking duration (1 hour)
        if (durationMinutes < MINIMUM_DURATION_MINUTES) {
          const error = new Error(
            `Minimum booking duration is ${MINIMUM_DURATION_MINUTES} minutes (1 hour)`,
          );
          error.statusCode = 400;
          throw error;
        }
        ...
        if (normalizedVoucherCode) {
          ...
          this._assertVoucherEligibility(
        ...
        for (const pod of podsToBook) {
          ...
          const bookingPrice = this._roundMoney(
            locked?.calculated_amount ?? pricePerPod,
          );
          bookingDocs.push({
            order_id: bookingOrder.id,
            user_id,
        ...
        const pricingDetailDocs = createdBookings.map((booking) => {
          ...
          return {
            ...
            calculated_amount: this._roundMoney(
              locked.calculated_amount ?? booking.total_price,
            ),
          };
        });

        const createdPricingDetails = await BookingPricingDetail.create(
          pricingDetailDocs,
          {
            session,
            ordered: true,
        ...
        for (const booking of createdBookings) {
          ...
          const createdTimeSlots = await TimeSlot.create(timeSlotDocs, {
            session,
            ordered: true,
          });
  ...
  _selectPodsForBooking(
    ...
    if (availablePods.length < podCount) {
      const error = new Error(
        `Chưa đủ ${podCount} pod trống trong khung giờ này.`,
      );
    ...
    if (
      floorPreference &&
      (floorPreference === "U" || floorPreference === "L")
    ) {
      const preferredPods = podInfo.filter(
        (info) => info.suffix === floorPreference,
      );
      ...
      if (preferredPods.length >= podCount) {
        ...
        if (!acceptMixedFloor) {
          const otherPods = podInfo.filter(
            (info) => info.suffix !== floorPreference,
          );
    ...
    if (requireAdjacent && podCount > 1) {
      ...
      for (const seg of allSegments) {
        ...
        if (take > 0) {
          ...
          const codeDisplays = seg.pods
            .slice(0, take)
            .map((p) => p.originalCode)
            .join(", ");
          breakdownDescriptions.push(
            `${take} pod khu ${seg.key} (${codeDisplays})`,
          );
    ...
    bestSelection.sort((a, b) => {
      const timeA = a.pod.last_cleaned_at
        ? new Date(a.pod.last_cleaned_at).getTime()
        : 0;
      const timeB = b.pod.last_cleaned_at
        ? new Date(b.pod.last_cleaned_at).getTime()
        : 0;
    ...
    try {
      ...
      if (actorRole === "manager") {
        ...
        visibleBookings = bookings.filter((booking) =>
          scopedPodIdSet.has(String(booking.pod_id)),
        );

        if (visibleBookings.length === 0) {
          const error = new Error(
            "You are not allowed to access this booking order",
          );
          error.statusCode = 403;
          throw error;
        }
      ...
      const clusterIds = [
        ...new Set(pods.map((pod) => pod.cluster_id).filter(Boolean)),
      ];
  ...
  async getAllBookingOrders(filters = {}, context = {}) {
    try {
      ...
      if (actorRole === "manager") {
        const managerScopedPodIds = this._normalizeIdList(
          context?.managerScope?.podIds,
        );
      ...
      if (requestedOrderIds !== null) {
        ...
        const existing =
          query.id && Array.isArray(query.id.$in) ? query.id.$in : null;
        query.id = existing
  ...
  async cancelBookingOrder(orderId, actor, options = {}) {
    ...
    try {
      const cancellationResult = await session.withTransaction(async () => {
        const order = await BookingOrder.findOne({ id: orderId }).session(
          session,
        );
        ...
        if (!isOwner) {
          const error = new Error(
            "Only order owner can cancel this booking order",
          );
          error.statusCode = 403;
          throw error;
        }
        ...
        // Get all bookings for this order
        const bookings = await Booking.find({ order_id: orderId }).session(
          session,
        );
        ...
        if (requestedBookingIds.length > 0) {
          ...
          const missingIds = requestedBookingIds.filter(
            (id) => !bookingMap.has(id),
          );
          if (missingIds.length > 0) {
            const error = new Error(
              `Invalid booking_ids: ${missingIds.join(", ")}`,
            );
            error.statusCode = 400;
            throw error;
          }
          ...
          targetBookings = bookings.filter(
        ...
        if (targetBookings.length === 0) {
          const error = new Error(
            "No active bookings available for cancellation",
          );
          error.statusCode = 400;
          throw error;
        }
        ...
        if (targetHasStartedOrCompleted) {
          const error = new Error(
            "Cannot cancel booking that is already in use or completed",
          );
          error.statusCode = 400;
          throw error;
        }

        const targetBookedBookings = targetBookings.filter(
          (booking) => booking.status === "BOOKED",
        );
        const targetBookingIds = targetBookedBookings.map(
          (booking) => booking.id,
        );
        ...
        if (order.status === "PENDING" && requestedBookingIds.length > 0) {
          const error = new Error(
            "PENDING order only supports full cancellation",
          );
          error.statusCode = 400;
          throw error;
        }
        ...
        if (order.status === "PENDING" && hasStartedOrCompletedBooking) {
          const error = new Error(
            "Cannot cancel order because one or more bookings are already in use or completed",
          );
          error.statusCode = 400;
          throw error;
        }
        ...
        const allCancelled = updatedBookings.every(
          (booking) => booking.status === "CANCELLED",
        );

        const nextOrderStatus = allCancelled
          ? "FULLY_CANCELLED"
          : "PARTIAL_CANCEL";
        ...
        let repricing = null;
        if (!allCancelled) {
          repricing = await this._recalculateOrderPricingAfterPartialCancel(
            order,
            session,
          );
        } else {
        ...
        const now = new Date();
        const refundSummary = await this._calculateRefundForBookings(
          targetBookedBookings,
          now,
          session,
        );
        const canRefundBefore48h =
          refundSummary.eligibleBookings.length === targetBookedBookings.length;
            ...
            [
              {
                ...
                status: "SUCCESS",
                provider_reference:
                  latestCharge?.provider_reference || "AUTO_REFUND_TO_WALLET",
              },
            ],
            { session },
          );
        ...
        return {
          ...
          refund: {
            ...
            eligible_booking_ids: refundSummary.eligibleBookings.map(
              (booking) => booking.id,
            ),
  ...
  async getPendingRefundRequests(filters = {}, context = {}) {
    ...
    const scopedOrderIds =
      await this._findOrderIdsByPodIds(managerScopedPodIds);
    if (scopedOrderIds.length === 0) {
      return {
        refunds: [],
        pagination: {
    ...
    const orderIds = [
      ...new Set(
        refunds.map((refund) => String(refund.order_id)).filter(Boolean),
      ),
    ];
    const orders =
      orderIds.length > 0
  ...
  async processRefundRequest(refundId, actor, options = {}) {
    ...
    if (refund.status !== "PENDING") {
      const error = new Error(
        `Refund request is already processed with status ${refund.status}`,
      );
      error.statusCode = 400;
      throw error;
    }

    const canAccess = await this._isOrderInManagerScope(
      refund.order_id,
      options.managerScope,
    );
    if (!canAccess) {
      const error = new Error(
  ...
  async applyVoucherToOrder(orderId, actor, payload = {}) {
    ...
    if (order.status !== "PENDING") {
      throw this._createError(
        "Voucher can only be applied to PENDING order",
        400,
      );
    }

    const existedRecord = await BookingVoucher.findOne({ order_id: order.id })
      .select("id")
      .lean();
    if (existedRecord) {
    ...
    try {
      const txResult = await session.withTransaction(async () => {
        const currentOrder = await BookingOrder.findOne({
          id: order.id,
        }).session(session);
        if (!currentOrder) {
          throw this._createError("Booking order not found", 404);
        ...
        const duplicate = await BookingVoucher.findOne({
  ...
  async removeVoucherFromOrder(orderId, actor) {
    ...
    if (order.status !== "PENDING") {
      throw this._createError(
        "Voucher can only be removed from PENDING order",
        400,
      );
    }
    ...
    try {
      const txResult = await session.withTransaction(async () => {
        ...
        const existing = await BookingVoucher.findOne({
          order_id: currentOrder.id,
        }).session(session);
        if (!existing) {
  ...
  async checkoutOrder(orderId, actor, options = {}) {
    ...
    if (!isOwner) {
      const error = new Error(
        "Only order owner can checkout this booking order",
      );
      error.statusCode = 403;
      throw error;
    }
    ...
    if (!["ALL_IN_ORDER", "SELECTED_BOOKINGS"].includes(scope)) {
      const error = new Error(
        "scope must be ALL_IN_ORDER or SELECTED_BOOKINGS",
      );
      error.statusCode = 400;
      throw error;
    }
    ...
    if (scope === "SELECTED_BOOKINGS") {
      ...
      if (mergedBookingIds.length === 0) {
        const error = new Error(
          "booking_ids is required when scope is SELECTED_BOOKINGS",
        );
        error.statusCode = 400;
        throw error;
      }
    ...
    for (const booking of bookings) {
      ...
      await reviewService.createReviewIfNotExists(booking).catch((err) => {
        console.error(
          `Failed to create review for booking ${booking.id}:`,
          err.message,
        );
      });
  ...
  async cleanupExpiredOrders() {
    try {
      ...
      expirationTime.setMinutes(
        expirationTime.getMinutes() - HOLD_EXPIRATION_MINUTES,
      );
  ...
  startCleanupJob(intervalMinutes = 10) {
    console.log(
      `Starting order cleanup job (interval: ${intervalMinutes} minute(s))`,
    );
  ...
  async getMyTodayBookings(userId) {
    try {
      // hôm nay
      ...
      const bookings = await Booking.find({
        ...
        checkin_state: "PENDING", // Chỉ lấy đơn chưa check-in
        status: { $ne: "CANCELLED" },
      })
        .sort({ start_time: 1 })
        .lean();
      ...
      return bookings.map((b) => {
        ...
        return {
          bookingId: b.id,
          orderId: b.order_id,
          podName: podMap[b.pod_id]?.name || "N/A",
